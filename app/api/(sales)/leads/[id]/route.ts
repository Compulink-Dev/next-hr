...
export async function GET(request: Request, { params: { id } }: { params: { id: string } }) {
  try {
    ...
    const rec = await db.lead.findUnique({ where: { id }, include: { company: { select: { name: true } }, owner: { select: { name: true } } } });
...
export async function PUT(request: Request, { params: { id } }: { params: { id: string } }) {
  try {
    ...
    const updated = await db.lead.update({
      where: { id },
      data: { name, email, phone, source, status, companyId: companyId ?? null },
    });
